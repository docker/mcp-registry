...
- 18 tools. 16 appear on a default connection; `list_keys` and `set_low_balance_alert` appear when their optional scopes are granted. Read tools never charge. `send_message` is the only tool that spends credit; the client must allow it first, and every reply reports the call's cost, the amount spent and the cap.
...
| `estimate_cost` | What a call would cost before you make it | auto, read-only |
...
| `get_usage`, `get_request` | Your requests, tokens and cost by model over a window; one request by id | auto, read-only |
